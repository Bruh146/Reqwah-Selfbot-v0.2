...
def startprint():
    ...
    print(f'''{Fore.RESET}
                       ...
                       {Fore.RED}REQWAH v{SELFBOT.__version__} | {Fore.CYAN}Logged in as: {Exeter.user.name}#{Exeter.user.discriminator} {Fore.CYAN}| ID: {Fore.CYAN}{Exeter.user.id}   
                       ...
                       {Fore.RED}Reqwah Discord Server {Fore.CYAN}https://discord.gg/BYBypkJ
    ''' + Fore.RESET)
...
def Init():
    ...
    try:
        ...
        os.system(f'title (Reqwah Selfbot) - Version {SELFBOT.__version__}')
        ...
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in Exeter.whitelisted_users.keys() and i.user.id in Exeter.whitelisted_users[
                    ...
                    await guild.ban(i.user, reason="req Anti-Nuke")
...
async def msgsniper(ctx, msgsniperlol=None):
    ...
    if str(msgsniperlol).lower() == 'true' or str(msgsniperlol).lower() == 'on':
        ...
        await ctx.send('Reqwah Message-Sniper is now **enabled**', delete_after=2)
    elif str(msgsniperlol).lower() == 'false' or str(msgsniperlol).lower() == 'off':
        Exeter.msgsniper = False
        await ctx.send('Reqwah Message-Sniper is now **disabled**', delete_after=2)
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(name="Reqwah Selfbot | Prefix: " + str(Exeter.command_prefix),
        ...
        embed.add_field(name="☢️ `HELP NUKE`", value="Shows all nuke commands", inline=False)
...
@Exeter.command(aliases=["REQWAHlogo", "phlogo"])
async def REQWAH(ctx, word1=None, word2=None):
    await ctx.message.delete()
    if word1 is None or word2 is None:
        await ctx.send("missing parameters")
        return
    endpoint = "https://api.alexflipnote.dev/REQWAH?text={text-1}&text2={text-2}".replace("{text-1}", word1).replace(
        "{text-2}", word2)
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"exeter_REQWAH_logo.png"))
    except:
        await ctx.send(endpoint)


@Exeter.command(aliases=["REQWAHcomment", 'phc'])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["message"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"exeter_REQWAH_comment.png"))
    except:
        await ctx.send(res["message"])
...
async def destroy(ctx):
    ...
    try:
        await ctx.guild.edit(
            name=RandString(),
            description="Reqwah selfbot smoked you",
            reason="Reqwah selfbot smoked you",
            icon=None,
            banner=None
        )
    except:
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="reqwah-selfbot-nuke")
    for _i in range(250):
        await ctx.guild.create_role(name="reqwah-selfbot-nuke", color=RandomColor())
...
async def massban(ctx):
    ...
    for user in users:
        try:
            await user.ban(reason="Reqwah selfbot smoked you")
        except:
            pass
...
async def masskick(ctx):
    ...
    for user in users:
        try:
            await user.kick(reason="Reqwah selfbot smoked you")
        except:
            pass
...
async def massrole(ctx):
    ...
    for _i in range(250):
        try:
            await ctx.guild.create_role(name="Reqwah selfbot nuke", color=RandomColor(), permissions=Permissions.all())
        except:
            try:
                await ctx.guild.create_role(name="Reqwah selfbot nuke", color=RandomColor())
            except:
...
async def spamchannels(ctx):
    ...
    for _i in range(250):
        try:
            await ctx.guild.create_text_channel("reqwah-selfbot-nuke")
        except:
            return
